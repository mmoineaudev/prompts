# Dungeon Crawler Visual — Reproduction Specification (v2)
...
**v2 changes over v1** (rulings of 2026-08-23): §18 BURN HP is **30 flat on NG 0** (was `enemyHpMultiplier(ngPlus,1,0)` = 90); save/continue explicitly IN scope (supersedes the stale v1 non-goal); elite Sharpshooter speed ×1.0; rat cap 6 = per-pack clamp; §24 replaced by a lean verification suite (dungeon-check + formula-check + browser smoke); palette hexes remain free graphics (no "match verbatim" gates).
...
| 7 | POISON_SWAMP | toxic marsh; torchless, acid pools, rat swarms (rat cap 6 = per-pack clamp, also bounded by the live-body cap) |
...
| Elite Archer | Sharpshooter | 2 | ×1.0 (same as base) | 2 | 2-arrow fan (±8°) |
...
- **BURN** (§18): HP `30 × (1 + 3·ngPlus)` — **30 flat on NG 0 at every level** (v2 ruling; supersedes the v1 `enemyHpMultiplier(ngPlus, 1, 0)` wording). Speed 2.6, damage 1, range 1.3, cooldown 1.4 s. Chases straight-line (sub-stepped, collision-resolved — does NOT phase).
...
## 24. Verification suite (lean v2 form)

The v1 suite was aspirational; **v2 keeps a lean, effort-efficient set** (v2 ruling — the implementer chooses the exact scripts). Minimum contract:

1. **Generator integrity check** (`node scripts/dungeon-check.mjs [seeds=40]`): seeded generator runs headless; mirror WorldBuilder collision boxes (thickness 0.3 ×0.6 depth, player radius 0.35); sample a 0.2 u walkable grid + 1-cell margin; BFS from the entrance; assert `escapes = 0`, `unreachableInside = 0`, `disconnected = 0` for every seed; report avg rooms / avg exit distance.
2. **Formula gates** (`node scripts/formula-check.mjs`): pure-function checks over Constants — tier thresholds 50/100/200/400/800 with the 11-point ceiling table (→ 0/0/1/1/2/2/3/3/4/4/5); sword damage ladder 2/2/3 → 7/7/8; swordSizeScale T0=1/T5=5 clamped at 5; attackSpeedFromSouls(1000)=2; orbDamageMultiplier(100)=3 / (1000)=21 and direct hit 6 / 42; explosion 5 @ 2 u; electric 5% chance, 20 u range, ×5 orb dmg; boss base HP 90 and the wealth/hearts halved-stack examples (49→90, 100→113, 300→158, 5 hearts→118, 100+5→154); spawn-weight columns sum to exactly 100 (7 entries); per-biome eligible room weight ≥ 100; enemyHpMultiplier overflow LINEAR; BURN HP 30×(1+3·ngPlus).
3. **Browser smoke** (headless Chromium via CDP): boot against the dev server; canvas + WebGL2 live; HUD ids present (`#orb-count` as THE single SOULS counter, `#perf-warning` hidden, `#biome-label`, `#timer`, `#hp-fill`, `#combo-pips`, `#weapon-slot`, `#stats-panel`); loading/title passed; timer advances; zero JS exceptions.

In-game invariants: memory stable over 3 descends; camera + sword survive regens; `window.game` exposed for QA.
...
- Save/continue is IN scope (v2 ruling; supersedes any earlier "no save" note): one death-save in localStorage (`dungeonCrawlerSave`) mirrored to disk by the save-server (`scripts/save-server.mjs`, port 5174, started by `launch.sh`); Load at startup restores meta-progression and restarts the saved level fresh. No mid-game pause.
