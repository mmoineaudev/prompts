...
6. Build level: `_generateDungeon()` → `_buildWorld()` → `_initLighting()` → `_initProps()` → `_initSmoke()` → `_initParticles()` → `_initRunes()` → `_initOrbs()` → `_initCombat()` → `_placeWaterPuddles()` → `_setupPlayerStart()`.
7. Welcome messages, bind toasts, `_emitLevelStart()` (`level:start` event), `_updateHUD()`.
8. Start clocks, `_showTitle()`, `_animate()`.
